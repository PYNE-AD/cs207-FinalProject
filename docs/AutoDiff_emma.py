...
class AutoDiff():
	...
	def __init__(self, eval_value, der_value, n=1, k=1, jacobian_value = np.array([[None]])):
		...
		self.der = None if der_value == None else self._convertNonArray(der_value)*self.val**0.0
		self.n = n
		self.jacobian = self._calcJacobian(k, n) if jacobian_value == None else jacobian_value*self.val**0.0
