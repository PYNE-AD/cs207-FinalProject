...
def test_cosh_results():
    X = autodiff(0.5, 2)
    f = ef.cosh(X)    
    assert coshAutoDiff.val == 1.1276259652063807
    assert coshAutoDiff.der == 1.0421906109874948
    assert coshAutoDiff.jacobian == 0.5210953054937474
...
def test_tanh_results():
    X = autodiff(0.5, 2)
    f = ef.tanh(X)    
    assert tanhAutoDiff.val == 0.46211715726000974
    assert tanhAutoDiff.der == 1.572895465931855
    assert tanhAutoDiff.jacobian == 0.7864477329659275
...
def test_log10_results():
    ...
    X = autodiff(0.5, 2)
    f = ef.log10(X)    
    assert tanhAutoDiff.val == -0.3010299956639812
    assert tanhAutoDiff.der == 1.737177927613007
    assert tanhAutoDiff.jacobian == 0.8685889638065035
